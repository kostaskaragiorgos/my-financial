...
class Expenses():
       ...
       # folders 
        ...
        if os.path.exists('expenses'+str(self.nowmonth)+'.csv') == False:
            with open('expenses'+str(self.nowmonth)+'.csv', 'a+') as f:
                ...
                thewriter.writerow(['Day','Amount','Description'])
                f.close()
    ...
    def addexp(self):
        ...
        if valam == 1 and valdes == 1:
            with open('expenses'+str(self.nowmonth)+'.csv', 'a+') as f:
                thewriter = csv.writer(f)
                thewriter.writerow([str(datetime.date.today().day),str(self.textamount.get(1.0,END)),self.textdes.get(1.0,END)])
                f.close()
    ...
    def helpmenu(self):
        msg.showinfo("Help", "Enter an amount(higher than zero) and a description and press Add Expense button")
